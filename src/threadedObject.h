    ...
    void threadedFunction()
    {
        while(isThreadRunning())
        {
            ...
            if(lock())
            {
				...
				string fadeCandyDir = directory + "fadecandy-master\\bin\\fcserver.exe";
				string fadeCandyConfig = directory + "fadecandy-master\\bin\\default.json";
				ofBackground(20);
				// ofSetLogLevel(OF_LOG_VERBOSE);
				string toSys = fadeCandyDir + " " + fadeCandyConfig;
				//cout << toSys << endl;
				char tab2[1024];
				strncpy(tab2, toSys.c_str(), sizeof(tab2));
